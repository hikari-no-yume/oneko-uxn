...
This port is based on version `oneko-1.2.sakura.5` from <http://www.daidouji.com/oneko/distfiles/oneko-1.2.sakura.5.tar.gz>. The original file is in `original/oneko-1.2.sakura.5.tar.gz`. The `oneko-uxn` directory contains the modified (uxn port) version.
